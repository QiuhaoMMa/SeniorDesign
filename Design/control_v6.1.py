...
def are_coordinates_equal(coord1, coord2, tolerance=0.02):
    ...
    while True:
        ...
        if os.path.isfile('output.txt') and os.path.getsize('output.txt') > 0:
            ...
            for i, object_pose in enumerate(object_poses, 1):
                ...
                reference_pose = [object_pose[0] - target_pose[0], object_pose[1] - target_pose[1], object_pose[2] - target_pose[2]] 

                alpha = np.round(np.arctan2(reference_pose[1], np.sqrt(reference_pose[0]**2 + reference_pose[1]**2)), 3)
                
                bot.arm.set_single_joint_position(joint_name='wrist_angle', position=alpha) 
                print()
                

                ## Waiting for spraying
                # Sending signal to start spraying
